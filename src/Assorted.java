...
public class Assorted {
    ...
    public static List<String> lineNumbering(List<String> list) {
        ...
        for (String element : list) {
            String lineWithNumber = lineNumber + ": " + element;
            result.add(lineWithNumber);
            lineNumber++;
        }
    ...
    public static int busStop(List<Integer[]> list) {
        ...
        for (Integer[] busStop : list) {
            int getOn = busStop[0];
            int getOff = busStop[1];
    ...
    public static int toBinary(List<Integer> list) {
        ...
        for (int i : list) {
            decimalValue += i * Math.pow(2, power);
            power--;
        }
        return decimalValue;
    }
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        for (int i = 0; i < list.size(); i++) {
            int motifs = list.get(i);
            int numOfTimes = 0;
            for (int j = 0; j < list.size(); j++) {
                if (list.get(j) == motifs) {
                    numOfTimes++;
                    if (numOfTimes > n) {
                        list.remove(j);
                    }
    ...
    public static List<String> wildWest(List<String> directions) {
        ...
        for (String direction : directions) {
            if (simplifiedDirections.size() > 0 && isOpposite(direction, simplifiedDirections.get(simplifiedDirections.size() - 1))) {
    ...
    public static int queueTime(List<Integer> queue, int tillsOpen) {
        ...
        for (int customerTime : queue) {
            int minIndex = findMinIndex(tills);
            int currentTillTime = tills.get(minIndex);
            tills.set(minIndex, currentTillTime + customerTime);
        }
        int maxTime = 0;
        for (int time : tills){
            if(time > maxTime){
